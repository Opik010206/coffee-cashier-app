    ...
    <tbody>
        @foreach ($meja as $c)
            <tr>
                ...
                <td class="d-flex">
                    {{-- Edit --}}
                    <button type="button" class="btn btn-success mx-2 " data-toggle="modal" data-target="#modal" data-mode="edit" data-id="{{ $c->id }}" data-no_meja="{{ $c->no_meja }}" data-kapasitas="{{ $c->kapasitas }}" data-status="{{ $c->status }}">
                    ...
                    <form action="meja/{{ $c->id }}" method="POST" class="">
                        @csrf
                        @method('DELETE')
