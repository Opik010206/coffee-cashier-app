<table class="table">
    ...
    <tbody>
        @foreach ($pemesanan as $c)
            <tr>
                <th>{{ $i = (!isset($i)?1:++$i) }}</th>
                <td>{{ $c->meja->no_meja }}</td>
                <td>{{ $c->tanggal_pemesanan }}</td>
                <td>{{ $c->jam_mulai }}</td>
                <td>{{ $c->jam_selesai }}</td>
                <td>{{ $c->nama_pemesan }}</td>
                <td>{{ $c->jumlah_pelanggan }}</td>
                <td class="d-flex">
                    {{-- Edit --}}
                    <button type="button" class="btn btn-success mx-2 " data-toggle="modal" data-target="#modal" data-mode="edit" data-id="{{ $c->id }}" data-meja_id="{{ $c->meja_id }}" data-tanggal_pemesanan="{{ $c->tanggal_pemesanan }}" data-jam_mulai="{{ $c->jam_mulai }}" data-jam_selesai="{{ $c->jam_selesai }}" data-nama_pemesan="{{ $c->nama_pemesan }}" data-jumlah_pelanggan="{{ $c->jumlah_pelanggan }}">
                    ...
                    <form action="pemesanan/{{ $c->id }}" method="POST" class="">
                        @csrf
                        @method('DELETE')
