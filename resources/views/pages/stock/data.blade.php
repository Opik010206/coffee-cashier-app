    ...
    <tbody>
        @foreach ($stock as $c)
            <tr>
                ...
                <td class="d-flex">
                    {{-- Edit --}}
                    <button type="button" class="btn btn-success mx-2 " data-toggle="modal" data-target="#modal" data-mode="edit" data-id="{{ $c->id }}" data-menu_id="{{ $c->menu_id }}" data-jumlah="{{ $c->jumlah }}">
                    ...
                    <form action="stock/{{ $c->id }}" method="POST" class="">
                        @csrf
                        @method('DELETE')
