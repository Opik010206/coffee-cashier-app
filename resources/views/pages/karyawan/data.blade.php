<table class="table table-responsive-md table-hover" id="tbl-karyawan">
    <thead>
      <tr>
        <th scope="col">ID</th>
    ...
    <tbody>
        @foreach ($karyawan as $c)
            <tr>
                ...
                <td class="">
                    {{-- Edit --}}
                    <button type="button" class="btn btn-success mx-2 btn-icon" data-toggle="modal" data-target="#modal" data-mode="edit" data-id="{{ $c->id }}" data-nama="{{ $c->nama }}" data-jenis_kelamin="{{ $c->jenis_kelamin }}" data-no_telp="{{ $c->no_telp }}" data-alamat="{{ $c->alamat }}" data-status_nikah="{{ $c->status_nikah }}" data-agama="{{ $c->agama }}" data-tempat_lahir="{{ $c->tempat_lahir }}" data-nip="{{ $c->nip }}" data-nik="{{ $c->nik }}" data-tanggal_lahir="{{ $c->tanggal_lahir }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-ballpen-filled" width="20" height="20" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <form action="karyawan/{{ $c->id }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button type="button" class="btn btn-danger btn-delete btn-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-trash-filled" width="20" height="20" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
