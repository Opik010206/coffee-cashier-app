    ...
    <tbody>
        @foreach ($karyawan as $c)
            <tr>
                ...
                <td class="d-flex">
                    {{-- Edit --}}
                    <button type="button" class="btn btn-success mx-2 " data-toggle="modal" data-target="#modal" data-mode="edit" data-id="{{ $c->id }}" data-nama="{{ $c->nama }}" data-jenis_kelamin="{{ $c->jenis_kelamin }}" data-no_telp="{{ $c->no_telp }}" data-alamat="{{ $c->alamat }}" data-status_nikah="{{ $c->status_nikah }}" data-agama="{{ $c->agama }}" data-tempat_lahir="{{ $c->tempat_lahir }}" data-nip="{{ $c->nip }}" data-nik="{{ $c->nik }}" data-tanggal_lahir="{{ $c->tanggal_lahir }}">
                    ...
                    <form action="karyawan/{{ $c->id }}" method="POST" class="">
                        @csrf
                        @method('DELETE')
