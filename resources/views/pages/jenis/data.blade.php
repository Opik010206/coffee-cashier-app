    ...
    <tbody>
        @foreach ($jenis as $c)
            <tr>
                <th>{{ $i = (!isset($i)?1:++$i) }}</th>
                <td>{{ $c->nama }}</td>
                <td>{{ $c->kategory->nama }}</td>
                <td class="">
                    {{-- Edit --}}
                    <button type="button" class="btn btn-success mx-2 btn-icon" data-toggle="modal" data-target="#modal" data-mode="edit" data-id="{{ $c->id }}" data-nama="{{ $c->nama }}" data-kategory_id="{{ $c->kategory_id }}">
